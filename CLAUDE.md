...
- `models.py` — Pydantic request/response models for both OpenAI and Anthropic formats. `ChatCompletionRequest` supports `response_format` (text/json_object/json_schema → system prompt injection via `get_json_instructions()`), `seed` (accepted, ignored), `stop` sequences (applied via post-processing), `tools`/`tool_choice` (function calling simulation). Models include `ToolDef`, `ToolCall`, `ToolCallFunction`, `FunctionDef` for OpenAI function calling format. `Usage` includes optional `total_cost_usd`, `duration_ms`, `num_turns` from SDK. Responses include `system_fingerprint` (deterministic per model).
- `function_calling.py` — OpenAI function calling simulation. `build_tools_system_prompt()` converts tool definitions to system prompt instructions. `parse_tool_calls()` extracts structured tool call blocks from Claude's response (supports ` ```tool_calls ``` ` format and bare JSON fallback). Multi-turn flows supported via `tool` role message conversion.
- `constants.py` — Single source of truth for model lists, tool names, rate limits, defaults. `MODEL_ALIASES` maps OpenAI/Gemini/DeepSeek model names to Claude equivalents (e.g. `gpt-4o` → `claude-sonnet-4-6`).
